...
-export([test/1, generateUsers/3, server/0, client/1, checkTableStatus/0, generateProfile/4]).
...
server() -> 
    receive
        {register, UserData} ->
            ...
            format("User data: ~p~n", [getUserData(maps:get(<<"Username">>, UserData))]),
            server();

        {updatePid, Username, Pid} ->
            ...
            % format("New User data: ~p~n", [getUserData(maps:get(<<"Username">>, UserData))]),
